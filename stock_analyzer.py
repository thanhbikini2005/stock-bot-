...
def volume_analysis(df):
    ...
    # Xác định flow ngắn gọn cho bảng tổng hợp
    if ratio_today >= 1.5 and change > 0:
        flow = "GOM HANG"
    elif ratio_today >= 1.5 and change < 0:
        flow = "XA HANG"
...
def format_all(results, now, vnindex_block):
    ...
    for a in sorted_r:
        vi    = a["vol_info"]
        verd  = verdict_label.get(a["verdict"], a["verdict"])
        trend = trend_label.get(a["trend"], "—")
        flow  = flow_short.get(vi.get("flow", ""), "⚪ BT")
        r20   = vi.get("ratio_today", 1.0)
        table_rows.append(
            f"  <b>{esc(a['symbol']):<5}</b>  {verd:<14}  {r20:.1f}x   {trend:<8}  {flow}"
        )
    ...
    summary_block = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━\n"
        "\n"
        "  <b>Mã      Lệnh            KL/TB   Xu hướng   Dòng tiền</b>\n"
        "  ────────────────────────────────────────────────\n"
        + "\n".join(table_rows)
    )
    ...
    # ── KẾT LUẬN TỔNG ──────────────────────────────────────────
    # Đọc kết luận VNIndex từ block text nếu có
    if vnindex_block:
        if "TÍCH CỰC" in vnindex_block:
            vn_sum = "✅ Thị trường TÍCH CỰC — xu hướng tăng, dòng tiền vào tốt."
            vn_act = "→ Ưu tiên mở vị thế mới với cổ phiếu mạnh."
            vn_warn = f"→ Nếu VNIndex về dưới MA20 → dừng mua mới."
        elif "TIÊU CỰC" in vnindex_block:
            vn_sum = "🔻 Thị trường TIÊU CỰC — áp lực bán còn mạnh."
            vn_act = "→ Hạn chế mua mới, ưu tiên bảo toàn vốn."
            vn_warn = "→ Chỉ giữ cổ phiếu có nền tảng tốt, cắt lỗ sớm."
        elif "THẬN TRỌNG" in vnindex_block:
            vn_sum = "⚠ Thị trường THẬN TRỌNG — tăng nhưng KL chưa xác nhận."
            vn_act = "→ Vào hàng từng phần, không all-in."
            vn_warn = "→ Theo dõi KL những phiên tới để xác nhận xu hướng."
        else:
            vn_sum = "⚠ Thị trường ĐI NGANG — chưa có hướng rõ ràng."
            vn_act = "→ Chờ tín hiệu rõ hơn trước khi hành động."
            vn_warn = "→ Ưu tiên quan sát, không giao dịch ồ ạt."
    else:
        vn_sum  = "❓ Không lấy được dữ liệu VNIndex hôm nay."
        vn_act  = "→ Phân tích dựa trên từng mã riêng lẻ."
        vn_warn = "→ Thận trọng hơn khi không có bức tranh thị trường chung."

    # Gợi ý hành động cụ thể từng nhóm
    action_lines = []
    if buy_syms:
        for sym in buy_syms:
            r = next((x for x in sorted_r if x["symbol"] == sym), None)
            if r:
                action_lines.append(
                    f"  • <b>{esc(sym)}</b> — <b>Có thể vào hàng</b>, TP {r['tp']:,.0f}  SL {r['sl']:,.0f}"
                )
    if sell_syms:
        for sym in sell_syms:
            action_lines.append(f"  • <b>{esc(sym)}</b> — <b>Thoát / giảm tỷ trọng ngay</b>")
    if watch_syms:
        action_lines.append(
            f"  • <b>{', '.join(esc(s) for s in watch_syms)}</b> — Chờ 1–2 phiên xác nhận thêm"
        )
    neutral_syms = [r["symbol"] for r in sorted_r if r["verdict"] == "TRUNG TÍNH"]
    if neutral_syms:
        action_lines.append(
            f"  • <b>{', '.join(esc(s) for s in neutral_syms)}</b> — Đứng ngoài quan sát"
        )
    ...
    conclusion_block = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━\n"
        "\n"
        f"  <b>VNIndex:</b>  {vn_sum}\n"
        f"  {vn_act}\n"
        f"  {vn_warn}\n"
        "\n"
    )
    if buy_syms:
        conclusion_block += f"  🟢 <b>Nên mua:</b>  {', '.join(esc(s) for s in buy_syms)}\n"
    if sell_syms:
        conclusion_block += f"  🔴 <b>Nên bán/thoát:</b>  {', '.join(esc(s) for s in sell_syms)}\n"
    if watch_syms:
        conclusion_block += f"  🟡 <b>Theo dõi thêm:</b>  {', '.join(esc(s) for s in watch_syms)}\n"
    if not buy_syms and not sell_syms:
        conclusion_block += "  ⚪ Không có tín hiệu rõ hôm nay — nghỉ ngơi, chờ phiên sau.\n"
    ...
    # Phân tách báo cáo hôm nay với hôm qua
    lines.append("\n\n— — — — — — — — — — — —\n\n")
